...
public interface GebruikerRepository extends CrudRepository<Gebruiker, Long> {
  ...
  @Query(value = "SELECT * FROM gebruiker AS g WHERE gebruiker_id IN(SELECT gebruiker_id FROM bestelling_snapshot AS p WHERE p.status = 'geplaatst' GROUP BY gebruiker_id HAVING SUM(total)> g.max_krediet)", nativeQuery = true)
